...
public class AllVAllWrapper {
    ...
    public Result[] allVsAllWithResults(ArrayList pairList, HashMap instMap) throws TopsStringFormatException {
    ...
    public void allVsAll(ArrayList pairList, HashMap instMap) throws TopsStringFormatException {
        //the domain names we are comparing are (pairKey, pairValue).
        String pair1, pair2;
        String[][] pairs = (String[][]) pairList.toArray(new String[0][]);
        String[] couple = new String[2];
    ...
    public ArrayList getNames(ArrayList examples) {
    ...
    public Result[] run(ArrayList names, ArrayList examples) {
        ArrayList pairList = new ArrayList();
        HashMap instMap = new HashMap(names.size());

        // map the names to the examples
        for (int i = 0; i < examples.size(); i++) {
            String nextLine = (String) examples.get(i);
            String nextName = (String) names.get(i);
        ...
        for (int i = 0; i < names.size(); i++) {
            for (int j = i + 1; j < names.size(); j++) {
                if (i != j) {
                    String[] miniContainer = {(String) names.get(i), (String) names.get(j)};
    ...
    public void printResultsAsOCInput(ArrayList names, Result[] results, PrintStream stream) {
    ...
    public static void main(String[] args) {
        ...
        ArrayList names = allVAllWrapper.getNames(examples);
